describe('Basic user flow for SPA ', () => {
  ...
  it('Test4: On first Entry page - checking page header title', async () => {
    ...
    const title = await page.evaluate(h1 => h1.textContent, h1);
  ...
  it('Test5: On first Entry page - checking <entry-page> contents', async () => {
    ...
    const entry = await page.evaluate(entryPage => entryPage.entry, entryPage);
  ...
  it('Test11: Clicking the back button, new URL should be homepage', async() => {
    await page.goBack();
  ...
  it('Test12: On HomePage - header title should be "Journal Entries"', async() => {
    const h1 = await page.$('h1'); 
    const title = await page.evaluate(h1 => h1.textContent, h1);
  ...
  it('Test14: On HomePage - verify url is correct when clicking on the second entry', async() => {
    await page.click('journal-entry + journal-entry');
    expect(page.url()).toBe('http://127.0.0.1:5500/#entry2');
  });

  // define and implement test15: Verify the title is current when clicking on the second entry
  it('Test15: On Seccond Entry - verify title is correct when clicking on the second entry', async() => {
    const h1 = await page.$('h1'); 
    const title = await page.evaluate(h1 => h1.textContent, h1);
  ...
  it('Test16: On Second Entry - verify entry page contents is correct when clicking on the second entry', async() => {
    const entryPage = await page.$('entry-page');
    const entry  = await page.evaluate(entryPage => entryPage.entry, entryPage);
    expect(entry).toEqual(
      {
  ...
  it('Test17: On Second Entry - go back to home page, then verify url is correct when clicking on the third entry', async() => {
    // Go back to homepage
    await page.goBack();
    // go to third journal entry
    await page.click('journal-entry + journal-entry + journal-entry');
    expect(page.url()).toBe('http://127.0.0.1:5500/#entry3');
  });

  // create your own test 18
  it('Test18: On Third Entry - verify title is correct when clicking on the third entry', async() => {
    const h1 = await page.$('h1'); 
    const title = await page.evaluate(h1 => h1.textContent, h1);
  ...
  it('Test16: On Third Entry - verify entry page contents is correct when clicking on the third entry', async() => {
    const entryPage = await page.$('entry-page');
    const entry  = await page.evaluate(entryPage => entryPage.entry, entryPage);
    expect(entry).toEqual(
      {
  ...
  it('Test20: On Third Entry - checking <body> element classes', async() => {
    const body = await page.$('body');
    const bodyClass = await page.evaluate(body => body.className, body);
    expect(bodyClass).toBe('single-entry');
  });
